...
reg [3:0] lrumem [0:SETCOUNT-1][0:SETCOUNT-1];//16way -> 4비트로 확장
...
reg [3:0] hitwaycore1,hitwaycore2; //히트된 way
//reg [31:0] dataoutreg; // 파이프라인 레지스터 1사이클 지연해서 dataout에 전달하는 역할이였는데
reg [31:0] pipelinedata [0:2]; //이러면 32비트짜리가 3개 있다. 위랑 차이는? 그냥 2차원배열로 선언?
//core1,core2로 나누거나 FSM으로 동적 조절도 가능.

//pipe라인 사용이유
//L3는 L2,L1가 1사이클 지연하던거에 비해 20~40사이클을 처리해야함. 따라서, 3단계 파이프라인 요구됨.
//[0] -> 태그비교 및 히트확인(hitwaycore1)
//[1] -> 데이터 메모리 읽기 (datamem 액세스)
//[2] -> 출력 안정화 및 전달 (dataoutcore1) -> 간소화로 작성
...
always @(posedge clk or negedge rstn) begin
    if(!rstn)begin
        ...
        if(hitcore1&&recore1 && mesimem[indexcore1][hitwaycore1]!=2'b00)begin
            pipelinedata[0]<=datamem[indexcore1][hitwaycore1][offsetcore1[5:2]]; //히트 데이터
            pipelinedata[1]<=pipelinedata[0];
            pipelinedata[2]<=pipelinedata[1];
            dataoutcore1 <= pipelinedata[2]; // 1사이클 지연
            validcore1 <=1;
        end else if (dramready && validmem[indexcore1][lrureplace]) begin
            dataoutcore1 <= datamem[indexcore1][lrureplace][offsetcore1[5:2]]; //미스 후 로드된 데이터
            validcore1<=1;
        end else begin
            validcore1 <=0;
        end
        ...
        if(hitcore2 && recore2 && mesimem[indexcore2][hitwaycore2]!=2'b00)begin
            pipelinedata[0]<=datamem[indexcore2][hitwaycore2][offsetcore2[5:2]]; //히트 데이터
            pipelinedata[1]<=pipelinedata[0];
            pipelinedata[2]<=pipelinedata[1];
            dataoutcore2 <= pipelinedata[2]; // 1사이클 지연
            validcore1 <=1;
        end else if (dramready && validmem[indexcore1][lrureplace]) begin
            dataoutcore1 <= datamem[indexcore1][lrureplace][offsetcore1[5:2]]; //미스 후 로드된 데이터
            validcore1<=1;
    ...
    if(!rstn)begin
        ...
        for(j=0; j<SETSIZE;j=j+1)begin
            if(validmem[indexcore1][j] && tagmem[indexcore1][j]==tagcore1)begin
                ...
                2'b00: begin //invalid
                    hitcore1<=0; // 무효 상태 -> no hit
                end
                endcase
                //LRU 업데이트
                lrumem[indexcore1][j]<=0; //0을 통해서 가장 최근에 사용됨으로 표시.
                //39번 코드 참고
                ...
                case (mesimem[indexcore2][j])
                    ...
                    2'b01: begin //shared
                        if (wecore2)begin
                            ...
                            dirtymem[indexcore2][j]<=1; 
                            mesimem[indexcore2][j]<=2'b11; /// modified
                            mesimem[indexcore1][j]<=2'b00; // invalid
                            //다른 코어 동일 데이터 무효화(간소화)
                        end
                    end
                    2'b00: begin //invalid
        ...
        if(!hitcore1 && (recore1||wecore1))begin//!hitcore1은 히트 실패
                                                //cache 미스는 데이터 요청있을때만 처리
                                                //
